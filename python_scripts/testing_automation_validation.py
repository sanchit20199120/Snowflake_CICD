...
def validation(arg):
    ...
    try:
        ...
        conn = snowflake.connector.connect(user = log_dsn_name['id'], password = log_dsn_name['secret'],
                                           account = log_dsn_name['host'], warehouse='COMPUTE_WH', schema = schema, database = database)

        print("SELECT * FROM " + input_table + " WHERE IS_ACTIVE = 'Y' AND IS_AUTOMATION_ENABLED = 'Y' AND WORKSTREAM_NAME ='" + workstream_name + "' AND DATA_SRC_NAME='" + data_source_name + "' AND JOB_ID = (SELECT MAX(JOB_ID::int) FROM " + input_table + " WHERE IS_ACTIVE ='Y' AND IS_AUTOMATION_ENABLED = 'Y' AND WORKSTREAM_NAME ='" + workstream_name + "' AND DATA_SRC_NAME='" + data_source_name + "')   ORDER BY  TEST_SUITES, ADO_TESTCASE_NO, TEST_STEP")
        ...
        dq_check_job_seq = pd.read_sql_query("SELECT " + database + ".QUALITY.TEST_AUTOMATION_JOB_SEQ.nextval",conn)
        ...
        for index, row in master_list.iterrows():
            ...
            output_job_seq = pd.read_sql_query("SELECT  " + database + ".QUALITY.TESTING_AUTOMATION_OUTPUT_SEQ.nextval", conn)
            ...
            try:
                sql_query = 'INSERT INTO ' + output_table + ' (OUTPUT_ID, JOB_ID, INPUT_JOB_ID, ISRT_DT_TM, WORKSTREAM_NAME,' \
                            ...
                            'TESTCASE_STATUS, TESTED_BY, COMMENT) VALUES'
            ...
            try:
                ...
                if success:
                    print(f"Successfully inserted {nrows} rows into {result_table}")
                else:
                    print(f"Failed to insert data in {result_table} ")
